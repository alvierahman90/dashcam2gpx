...
pattern = re.compile(r"(\d+)\s*KM/HN[: ]?(\d+\.\d+)\s*E[: ]?(\d+\.\d+)")
    ...
    while True:
        ...
        if frame_count % frame_interval == 0:
            ...
            text = pytesseract.image_to_string(
                thresh,
                config="--psm 7 -c tessedit_char_whitelist=0123456789:.NEKM/H "
            )

            gps_match = pattern.search(text)

            if gps_match:
                speed, lat, lon = gps_match.groups()
                speed, lat, lon = int(speed), float(lat), float(lon)
                print(f"Match {frame_count}/{total_frames}: {lat}/{lon}, {speed} km/h")
...
if __name__ == "__main__":
    ...
    parser.add_argument("video_path", help="Path to dashcam video file (e.g., dashcam.mp4)")
